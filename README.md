...
> **A receipts-first entertainment monitoring toolkit for Japanese entertainment fans.**
...
> Built by a fan who prefers receipts over rumors. 🎬🔎  
> 噂より根拠を重視する、オタクによる小さな試みです。  
> 一个重证据、轻传闻的日娱爱好者开源小项目。
...
- `watchlists/`：通用艺人监测配置示例
- `examples/`：示例发现记录和分级报告
...
jent-radar init \
  ...
  --aliases "Snow Man,しょっぴー,なべしょ" \
  --window "2026年秋冬" \
  --output watchlists/shota-watanabe.json
```

仓库中保留的是通用配置示例；以上命令会在本地生成新的艺人配置文件。
...
> **为日娱爱好者设计的公开信息 OSINT 工作流。**  
> An open-source OSINT workflow for Japanese entertainment fans.
